...
def main(argv):
    ...
    dir_to_backup = path.join('/Users/mcompton/Work/Simpson/src', repo)
    ...
    uncommitted_diff_dirpath = path.join(backup_base_dirpath, 'uncommittedDiff')
    unpushed_commits_dirpath = path.join(backup_base_dirpath, 'unpushedCommits/')


    rev_hash, rev_date = hg_newest_unpushed_commit(dir_to_backup)
    ...
    if rev_hash != None:
        ...
        if not path.exists(bundle_filename):
            hg_bundle_unpushed_commits(dir_to_backup, bundle_filename)

    files_to_backup = hg_list_uncommitted_files(dir_to_backup)

    rsync(dir_to_backup, files_to_backup, uncommitted_files_dirpath)
